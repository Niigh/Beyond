...
class EmbedBuilder {
    ...
    getWeaponEmbed(discordID, avatar, weaponSlot, weapon, weaponHash, weaponState, bungieTag=undefined) {
        ...
        if (killTracker != undefined) {
            trackers += `${destinyEmote.getKillTrackerEmote('PVE')} Enemies Defeated : ${killTracker[0].progress}\n`;
        }
        ...
        if(crucibleTracker != undefined) {
            trackers += `${destinyEmote.getKillTrackerEmote('PVP')} Crucible Opponents Defeated : ${crucibleTracker[0].progress}\n`;
        }
        const catalystTracker = destinyDB.getObjectivePlugByName(weapon.plugObjectives.data.objectivesPerPlug, 'Catalyst');
        if(catalystTracker != undefined) {
            trackers += `${destinyEmote.getKillTrackerEmote('PVE')} Enemies Defeated : ${catalystTracker[0].progress}\n`;
        }
        ...
        // Perks
        const perksField = {name: `PERKS`, value: `\u200b\n`, inline: true};
        fieldsArray.push(perksField);

        // Stats
        const statsField = {name: `STATS`, value: `\u200b\n`, inline: true};
        fieldsArray.push(statsField);

        // Intrinsic
        const intrinsicField = {name: `INTRINSIC`, value: `\u200b\n`, inline: false};
