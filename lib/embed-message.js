...
class EmbedBuilder {
    ...
    getArmorEmbed(discordID, avatar, armorSlot, armor, armorHash, armorState, bungieTag=undefined) {
        destinyDB.loadDB();
        ...
        const armorItem = destinyDB.getDetailedEquippedItem(armorHash);
        var source = '';
        if(armorItem.collectibleHash != undefined) {
            source = `*${destinyDB.getSource(armorItem.collectibleHash)}*`;
        }
        const sockets = destinyDB.getSockets(armor.sockets.data.sockets);

        var embedTitle = `${destinyEmote.getItemLockStateEmote(armorState.includes('Locked'))}${armorItem.displayProperties.name}`;

        // Fields
        const fieldsArray = [];

        // Energy
        const energy = [armor.instance.data.energy.energyCapacity, armor.instance.data.energy.energyUsed, armor.instance.data.energy.energyUnused];
        const energyField = {
            name: `${destinyEmote.getElementEmote(bungieAPI.getEnergyType(armor.instance.data.energy.energyType))} **${armor.instance.data.primaryStat.value}**`,
            value: 
            `**USED : ${energy[1]} / ${energy[0]}**\n`+
            `${destinyEmote.getElementGaugeEmote(bungieAPI.getEnergyType(armor.instance.data.energy.energyType),energy[0],energy[1])}\n\u200b\n`,
            inline: false
        };
        fieldsArray.push(energyField);

        //Stats
        const statsField = {name: `STATS`, value: ``, inline: false};

        const mobility = destinyDB.getStatsByName(armor.stats.data.stats, 'Mobility').value;
        const resilience = destinyDB.getStatsByName(armor.stats.data.stats, 'Resilience').value;
        const recovery = destinyDB.getStatsByName(armor.stats.data.stats, 'Recovery').value;
        const discipline = destinyDB.getStatsByName(armor.stats.data.stats, 'Discipline').value;
        const intellect = destinyDB.getStatsByName(armor.stats.data.stats, 'Intellect').value;
        const strength = destinyDB.getStatsByName(armor.stats.data.stats, 'Strength').value;

        const statTotal = mobility + resilience + recovery + discipline + intellect + strength;
        statsField.name += ` **|** Total: ${statTotal}`;

        statsField.value += `${destinyEmote.getStatEmote('Mobility')} ${mobility}`+` **|** `;
        statsField.value += `${destinyEmote.getStatEmote('Resilience')} ${resilience}`+` **|** `;
        statsField.value += `${destinyEmote.getStatEmote('Recovery')} ${recovery}`+` **|** `;
        statsField.value += `${destinyEmote.getStatEmote('Discipline')} ${discipline}`+` **|** `;
        statsField.value += `${destinyEmote.getStatEmote('Intellect')} ${intellect}`+` **|** `;
        statsField.value += `${destinyEmote.getStatEmote('Strength')} ${strength}`;

        fieldsArray.push(statsField);

        // Exotic Perks
        if(armorItem.inventory.tierTypeName == 'Exotic'){
            const intrinsicField = {name: ``, value: ``, inline: false};
            const intrinsic = destinyDB.getExoticArmorPerk(sockets);

            intrinsicField.name = `${destinyEmote.getExoticArmorPerk(intrinsic.name)} ${intrinsic.name}`;
            intrinsicField.value = `${intrinsic.description}`;

            fieldsArray.push(intrinsicField);
        }

        // Mods
        const modsField = {name: `MODS`, value: ``, inline: true};
        var modsFieldValue = "";
        if (sockets == undefined) {
            dbg('No mod equipped.');
            modsFieldValue = "No mod equipped.\n";
        } else {
            for (let i = 0; i < 4; i++) {
                const mod = sockets[i];
                if(mod.displayProperties.name != 'Empty Mod Socket') {
                    modsFieldValue += `[${mod.displayProperties.name}](https://www.light.gg/db/items/${mod.hash}/) **| ${mod.plug.energyCost.energyCost}** ${destinyEmote.getGhostModEnergy(mod.plug.energyCost.energyCost)}\n`;
                }
            }
        }
        modsField.value += modsFieldValue;
        fieldsArray.push(modsField);

        const cosmeticsField ={name: `COSMETICS`, value: `\u200b`, inline: false};

        const shader = destinyDB.getShader(armor.sockets.data.sockets);
        if(shader != undefined) {
            cosmeticsField.value += `**[${shader.displayProperties.name}](https://www.light.gg/db/items/${shader.hash}/)**\n`;
        }

        const ornement = destinyDB.getWeaponOrnement(armor.sockets.data.sockets);
        if(ornement != undefined) {
            cosmeticsField.value += `**[${ornement.displayProperties.name}](https://www.light.gg/db/items/${ornement.hash}/)**`;
        }
        ...
        const armorEmbed = {
            ...
            title: embedTitle,
            url: `https://www.light.gg/db/items/${armorHash}/`,
            description:
                `${source}\n`,
            fields: fieldsArray,
    ...
    getExtendedArmorEmbed(discordID, avatar, armorSlot, armor, armorHash, armorState, bungieTag=undefined) {
        destinyDB.loadDB();
        ...
        var image = bungieAPI.buildURLAsset(armorItem.screenshot);
        ...
        const statsField = {name: `STATS`, value: ``, inline: false};
        const statsArray = [];
        const statsBaseArray = [];

        const mobility = destinyDB.getStatsByName(armor.stats.data.stats, 'Mobility').value;
        statsBaseArray.push(mobility);

        const resilience = destinyDB.getStatsByName(armor.stats.data.stats, 'Resilience').value;
        statsBaseArray.push(resilience);

        const recovery = destinyDB.getStatsByName(armor.stats.data.stats, 'Recovery').value;
        statsBaseArray.push(recovery);

        const discipline = destinyDB.getStatsByName(armor.stats.data.stats, 'Discipline').value;
        statsBaseArray.push(discipline);
    
        const intellect = destinyDB.getStatsByName(armor.stats.data.stats, 'Intellect').value;
        statsBaseArray.push(intellect);

        const strength = destinyDB.getStatsByName(armor.stats.data.stats, 'Strength').value;
        statsBaseArray.push(strength);

        const statTotal = mobility + resilience + recovery + discipline + intellect + strength;
        statsField.name += ` **|** Total: ${statTotal}`;

        if(armorState.includes('Masterwork')) {
            statsBaseArray.forEach((stat, index) => {
                statsBaseArray[index] -= 2;
            });
        }

        const statMod = destinyDB.getStatMod(sockets);

        statsArray.push(`${destinyEmote.getStatEmote('Mobility')} ${mobility}`);
        statsArray.push(`${destinyEmote.getStatEmote('Resilience')} ${resilience}`);
        statsArray.push(`${destinyEmote.getStatEmote('Recovery')} ${recovery}`);
        statsArray.push(`${destinyEmote.getStatEmote('Discipline')} ${discipline}`);
        statsArray.push(`${destinyEmote.getStatEmote('Intellect')} ${intellect}`);
        statsArray.push(`${destinyEmote.getStatEmote('Strength')} ${strength}`);

        if (statMod != undefined) {
            for (let i = 0; i < statMod.length; i++) {
                switch (statMod[i][0]) {
                    case 'Mobility':
                        statsBaseArray[0] -= statMod[i][1];
                        break;
                    case 'Resilience':
                        statsBaseArray[1] -= statMod[i][1];
                        break;
                    case 'Recovery':
                        statsBaseArray[2] -= statMod[i][1];
                        break;
                    case 'Discipline':
                        statsBaseArray[3] -= statMod[i][1];
                        break;
                    case 'Intellect':
                        statsBaseArray[4] -= statMod[i][1];
                        break;
                    case 'Strength':
                        statsBaseArray[5] -= statMod[i][1];
                        break;
                    default:
                        break;
                }
            ...
            for (let i = 0; i < statMod.length; i++) {
                switch (statMod[i][0]) {
                    case 'Mobility':
                        if(!statsArray[0].includes('|')) {
                            statsArray[0] += ` **| ${statsBaseArray[0]}** + ${statMod[i][1]}`;
                        } else {
                            statsArray[0] += ` + ${statMod[i][1]}`;
                        }
                        break;
                    case 'Resilience':
                        if(!statsArray[1].includes('|')) {
                            statsArray[1] += ` **| ${statsBaseArray[1]}** + ${statMod[i][1]}`;
                        } else {
                            statsArray[1] += ` + ${statMod[i][1]}`;
                        }
                        break;
                    case 'Recovery':
                        if(!statsArray[2].includes('|')) {
                            statsArray[2] += ` **| ${statsBaseArray[2]}** + ${statMod[i][1]}`;
                        } else {
                            statsArray[2] += ` + ${statMod[i][1]}`;
                        }
                        break;
                    case 'Discipline':
                        if(!statsArray[3].includes('|')) {
                            statsArray[3] += ` **| ${statsBaseArray[3]}** + ${statMod[i][1]}`;
                        } else {
                            statsArray[3] += ` + ${statMod[i][1]}`;
                        }
                        break;
                    case 'Intellect':
                        if(!statsArray[4].includes('|')) {
                            statsArray[4] += ` **| ${statsBaseArray[4]}** + ${statMod[i][1]}`;
                        } else {
                            statsArray[4] += ` + ${statMod[i][1]}`;
                        }
                        break;
                    case 'Strength':
                        if(!statsArray[5].includes('|')) {
                            statsArray[5] += ` **| ${statsBaseArray[5]}** + ${statMod[i][1]}`;
                        } else {
                            statsArray[5] += ` + ${statMod[i][1]}`;
                        }
                        break;
                    default:
                        break;
        ...
        if(armorState.includes('Masterwork')) {
            statsArray.forEach((stat, index) => {
                if (!statsArray[index].includes('|')) {
                    statsArray[index] += ` **| ${statsBaseArray[index]}**`;
                }
                statsArray[index] += ` + *2*`;
            });
        }
        ...
        // Mods
        const modsField = {name: `MODS`, value: ``, inline: false};
        var modsFieldValue = "";
        if (sockets == undefined) {
            dbg('No mod equipped.');
            modsFieldValue = "No mod equipped.\n";
